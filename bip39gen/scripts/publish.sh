...
($(isNullOrEmpty "$VERSION")) && ( echoErr "ERROR: Bip39GenVersion was NOT found in '$CONSTANS_FILE' !" && sleep 5 && exit 1 )
...
function pcgRelease() {
    ...
    if [ "$PLATFORM" != "windows" ] ; then
        local RELEASE_PATH="${RELEASE_DIR}/bip39gen_${VERSION}_${ARCH}.deb"
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/bip39gen"
        ...
        cp -fv "$RELEASE_PATH" ./bin/bip39gen-${PLATFORM}-${ARCH}.deb
    else
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/bip39gen.exe"
        # deb is not supported on windows, simply copy the executables
        cp -fv $BIN_PATH/bip39gen.exe ./bin/bip39gen-${PLATFORM}-${ARCH}.exe
    fi
}
...
echoInfo "INFO: Sucessfully published bip39gen deb packages into ./bin"

...
function pcgRelease() {
    ...
    if [ "$PLATFORM" != "windows" ] ; then
        local RELEASE_PATH="${RELEASE_DIR}/priv-validator-key-gen_${VERSION}_${ARCH}.deb"
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/priv-validator-key-gen"
        ...
        cp -fv "$RELEASE_PATH" ./bin/priv-validator-key-gen-${PLATFORM}-${ARCH}.deb
    else
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/priv-validator-key-gen.exe"
        # deb is not supported on windows, simply copy the executables
        cp -fv $BIN_PATH/priv-validator-key-gen.exe ./bin/priv-validator-key-gen-${PLATFORM}-${ARCH}.exe
    fi
}
...
echoInfo "INFO: Sucessfully published priv-validator-key-gen deb packages into ./bin"

...
($(isNullOrEmpty "$VERSION")) && ( echoErr "ERROR: IpfsApiVersion was NOT found in '$CONSTANS_FILE' !" && sleep 5 && exit 1 )
...
function pcgRelease() {
    ...
    if [ "$PLATFORM" != "windows" ] ; then
        local RELEASE_PATH="${RELEASE_DIR}/ipfs-api_${VERSION}_${ARCH}.deb"
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/ipfs-api"
        ...
        cp -fv "$RELEASE_PATH" ./bin/ipfs-api-${PLATFORM}-${ARCH}.deb
    else
        ./scripts/build.sh "${PLATFORM}" "${ARCH}" "$BIN_PATH/ipfs-api.exe"
        # deb is not supported on windows, simply copy the executables
        cp -fv $BIN_PATH/ipfs-api.exe ./bin/ipfs-api-${PLATFORM}-${ARCH}.exe
    fi
}
...
echoInfo "INFO: Sucessfully published ipfs-api deb packages into ./bin"